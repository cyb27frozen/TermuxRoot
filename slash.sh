...
if [ -d /sdcard/download/TermuxRoot-Script ]; then
    echo -e "${YELLOW}Running script...${NC}"
else
    echo -e "${RED}Error: The folder /sdcard/download/TermuxRoot-Script
    doesn't exist.
...
printf "Moving files...\n"
sleep 1
mv -f sources.list trusted.gpg /data/data/com.termux/files/usr/etc/apt
...
printf "\n${GREEN}All files moved successfully. Enjoy!${NC}\n"
...
if [ "$answer" != "${answer#[Yy]}" ]; then
    ...
    printf "\n${GREEN}Okay, skipping packages update.${NC}\n"
fi
...
printf "\n${RED}Don't forget the like & subscribe for more!${NC}👇😉\n"
